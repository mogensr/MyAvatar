...
async def debug_admin_users():
    ...
    try:
        admins = execute_query(
            "SELECT id, username, email, is_admin, created_at FROM users WHERE is_admin = true",
            fetch_all=True
        )
...
async def home_page(request: Request):
    ...
    try:
        ...
        # CRITICAL FIX: Redirect to enhanced login page instead of serving index.html
        # This ensures app.myavatar.dk shows the login page with MyAvatars.dk logo
...
async def login_user(request: Request):
    ...
    try:
        ...
        # 🔧 Prioritize hashed_password over legacy password column
        ...
        # 🔥 Wake up HF Space on login for faster BackgroundFX processing
        try:
            from app.services.hf_space_warmer import HFSpaceWarmer
            hf_warmer = HFSpaceWarmer()
            hf_warmer.trigger_warm_on_login(user["id"])
        except Exception as hf_error:
            logger.warning(f"⚠️ HF Space warming failed on login for user {user['id']}: {hf_error}")
