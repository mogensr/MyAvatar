...
PURPOSE: Authentication service handling passwords, tokens, and sessions
UPDATED: 2025-07-18 - Fixed JWT token validation for jose/pyjwt compatibility
"""
import bcrypt
import uuid
import time
...
# FIXED: Import JWT handling with proper exception handling
try:
    from jose import jwt
    JWT_AVAILABLE = True
    JWT_LIBRARY = "jose"
    logger = logging.getLogger(__name__)
    logger.info("Using jose library for JWT")
except ImportError:
    try:
        ...
        JWT_LIBRARY = "pyjwt"
        logger = logging.getLogger(__name__)
        logger.info("Using PyJWT library for JWT")
    except ImportError:
        JWT_AVAILABLE = False
        JWT_LIBRARY = "none"
        class jwt:
...
class AuthService:
    ...
    def validate_token(self, token: str) -> Optional[Dict]:
        """FIXED: Validate JWT token with proper exception handling"""
        try:
            if JWT_LIBRARY == "jose":
                # Using jose library
                ...
                logger.warning("No JWT library available, token validation failed")
                return None
                
        except Exception as e:
            # FIXED: Handle both jose and pyjwt exceptions properly
            error_name = e.__class__.__name__
            if error_name in ['ExpiredSignatureError', 'InvalidTokenError', 'DecodeError']:
                logger.debug(f"Token validation failed: {error_name}")
                return None
            else:
