...
def verify_token(token: str, token_type: str = "access") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"🔍 TOKEN DEBUG: Complete payload = {payload}")
        print(f"🔍 TOKEN DEBUG: Expected type = {token_type}")
        print(f"🔍 TOKEN DEBUG: Actual type = {payload.get('type')}")
        print(f"🔍 TOKEN DEBUG: All payload keys = {list(payload.keys())}")
        
        actual_type = payload.get("type")
        
        # 🔧 FIX: Handle legacy tokens without type field for backward compatibility
        if token_type == "access" and actual_type is None:
            print(f"⚠️ TOKEN DEBUG: No type field found, assuming access token for backward compatibility")
            # Allow tokens without type field for BackgroundFX compatibility
        elif actual_type != token_type:
            print(f"❌ TOKEN DEBUG: Type mismatch - expected '{token_type}', got '{actual_type}'")
            return None
        ...
        if exp:
            ...
            if current_time > exp:
                print(f"❌ TOKEN DEBUG: Token expired")
                return None
        
        print(f"✅ TOKEN DEBUG: Token valid for user {payload.get('sub') or payload.get('user_id')}")
        return payload

    except JWTError as e:
        print(f"❌ TOKEN DEBUG: JWT validation error - {str(e)}")
        return None
    except Exception as e:
        print(f"❌ TOKEN DEBUG: Token verification exception - {str(e)}")
        return None
...
def get_current_user(request: Request) -> Optional[Dict[str, Any]]:
    ...
    try:
        # 🔍 DEBUG: Check all cookies (Claude's debugging)
        print(f"🔍 COOKIE DEBUG: All cookies = {dict(request.cookies)}")
        
        # Try to get token from cookie first
        token = request.cookies.get("access_token")
        print(f"🔍 COOKIE DEBUG: access_token = {token}")

        # Fallback to Authorization header
        if not token:
            auth_header = request.headers.get("Authorization")
            print(f"🔍 COOKIE DEBUG: Authorization header = {auth_header}")
            if auth_header and auth_header.startswith("Bearer "):
                token = auth_header.split(" ")[1]
                print(f"🔍 COOKIE DEBUG: Token from header = {token}")

        if not token:
            print("❌ COOKIE DEBUG: No access_token cookie or Authorization header found")
            return None

        # Verify token (now handles legacy tokens)
        payload = verify_token(token, "access")
        if not payload:
            print("❌ COOKIE DEBUG: Token verification failed")
            return None
        
        print(f"✅ COOKIE DEBUG: User authenticated - {payload.get('sub') or payload.get('user_id')}")

        # 🔧 FIX: Handle both legacy (user_id) and new (sub) token formats
        user_identifier = payload.get("sub") or str(payload.get("user_id"))
        if not user_identifier:
            print("❌ COOKIE DEBUG: No user identifier found in token")
            return None

        # Normalize payload format for consistent usage
        normalized_payload = {
            "sub": user_identifier,
            "user_id": payload.get("user_id") or int(user_identifier),
            "exp": payload.get("exp"),
            "type": payload.get("type", "access")  # Default to access for legacy tokens
        }

        # Get user from database
        # Check if this is a legacy token (has user_id) or new token (has sub with username)
        if payload.get("user_id") is not None:
            # Legacy token: lookup by user_id
            print(f"🔍 DATABASE DEBUG: Looking up user by ID: {payload.get('user_id')}")
            user_raw = execute_query(
            ...
            # New token: lookup by username (sub field)
            print(f"🔍 DATABASE DEBUG: Looking up user by username: {user_identifier}")
            user_raw = execute_query(
        ...
        user['token_exp'] = normalized_payload.get('exp')

        return user

    except Exception as e:
        print(f"❌ COOKIE DEBUG: Exception in get_current_user - {str(e)}")
        return None
