...
async def home_page(request: Request):
    ...
    try:
        ...
        if user:
            ...
            # Previously this was always redirecting to /dashboard
            # Now we check if user is admin and redirect to /admin appropriately
...
async def login_user(request: Request):
    ...
    try:
        ...
        # 🔧 FIXED: Admin redirect logic is correct here
        # This was working - the issue was in the home page redirect above
        if user.get("is_admin", 0) == 1:
...
async def test_routes():
    ...
    return {
        ...
        "admin_redirect_fix": {
            "status": "FIXED",
            "issue": "Home page was always redirecting to /dashboard instead of checking admin status",
            "fix": "Added proper admin check in home_page(), login_page(), and register_page() routes",
            "affected_lines": ["Line 626: home_page()", "Line 693: login_page()", "Line 879: register_page()"],
            "comment": "🔧 ADMIN REDIRECT FIX - Now properly checks if user.is_admin == 1 and redirects to /admin"
        },
        "features": [
...
async def get_completed_videos_api(request: Request):
    """Get all completed videos for the user, including local and HeyGen videos."""
    ...
    try:
        ...
        # Direct SQL query - get all completed videos with a valid video_path
        ...
        # Debug: Show ALL videos in database with their avatar_id
        try:
            conn_dbg = psycopg2.connect(os.getenv('DATABASE_URL'))
            cur_dbg = conn_dbg.cursor(cursor_factory=RealDictCursor)
            cur_dbg.execute("SELECT id, title, avatar_id, status, created_at FROM videos ORDER BY created_at DESC LIMIT 10")
            debug_videos = cur_dbg.fetchall()
            conn_dbg.close()
            logger.info(f"[DEBUG] Last 10 videos: {debug_videos}")
        except Exception as dbg_e:
            logger.error(f"[DEBUG] Failed to fetch debug video list: {dbg_e}")
        ...
        for video in videos:
            ...
            # Mark if this is a local video (not a URL)
            video_dict['is_local_video'] = not (str(video_path).startswith('http') if video_path else False)
...
async def test_videos_route():
    """Test route to verify web_routes.py is working"""
    return {"message": "web_routes.py is working", "route": "/test-videos"}


@router.get("/avatar-selection", response_class=HTMLResponse)
async def avatar_selection_page(request: Request):
    """Avatar selection page - allows users to choose avatars"""
    try:
        user = get_current_user_fixed(request)
        if not user:
            return RedirectResponse(url="/login", status_code=302)
        
        logger.info(f"👤 AVATAR SELECTION - User {user.get('username')} accessing avatar selection page")
        
        # Get trial status if user is a trial user
        is_trial = user.get("account_type") == "trial"
        
        # Get demo avatars from portal.trial_management
        male_demo_avatar = {
            "name": "Adrian",
            "image_url": "/static/img/avatars/adrian.jpg"
        }
        female_demo_avatar = {
            "name": "Abigail",
            "image_url": "/static/img/avatars/abigail.jpg"
        }
        
        # Try to get from trial_manager if available
        try:
            from portal.trial_management import trial_manager
            male_demo_avatar = {
            ...
            logger.warning("⚠️ Could not import trial_manager, using default demo avatars")
        
        # Get user's current avatars
        user_avatars = []
        try:
            avatars = execute_query(
                "SELECT * FROM user_avatars WHERE user_id = %s ORDER BY is_default DESC, created_at DESC",
                (int(user["id"]),),
                fetch_all=True
            )
            ...
            if avatars:
                for avatar in avatars:
                    ...
                    user_avatars.append({
                        ...
                        'name': avatar_dict.get('avatar_name', 'Unnamed Avatar'),
                        'image_url': avatar_dict.get('avatar_image_url', ''),
                        'avatar_id': avatar_dict.get('avatar_id', ''),
                        'is_default': avatar_dict.get('is_default', False)
                    })
        except Exception as e:
        ...
        context = {
            ...
            "is_trial": is_trial,
            "user_id": user["id"],
            "male_demo_avatar": male_demo_avatar,
            "female_demo_avatar": female_demo_avatar,
            "user_avatars": user_avatars,
            "avatar_count": len(user_avatars)
        }
        
        return templates.TemplateResponse("avatar_selection.html", context)
    
    except Exception as e:
        logger.error(f"❌ Error in avatar selection page: {e}")
        import traceback
        traceback.print_exc()
        return RedirectResponse(url="/dashboard", status_code=302)
...
async def videos_page(request: Request):
    """Videos page - renders videos.html template with user's video data"""
    try:
        user = get_current_user_fixed(request)
        if not user:
            return RedirectResponse(url="/login", status_code=302)
        ...
        # Get user videos directly from database (safe approach)
        videos = []
        try:
        ...
        context = {
            ...
            "videos": processed_videos,
            "total_videos": total_videos,
            "user_avatars": [],  # Empty for now to avoid errors
            "avatar_count": 0,
            "credits_remaining": user.get("credits_remaining", 0),
            "max_credits": user.get("max_credits", 100),
            "max_videos_per_user": 7,  # Safe default
            "user_video_count": total_videos,
            "vacation_mode": False,  # Safe default
            "video_limit_reached": False  # Safe default
        }
        
        logger.info(f"🎬 VIDEOS PAGE: Rendering videos.html with {total_videos} videos")
...
# =============================================================================
# TEXT-TO-VIDEO AND VOICE-TO-VIDEO ROUTES
# =============================================================================

# TEXT-TO-VIDEO ROUTE REMOVED - Now handled in main.py to avoid conflicts

# REMOVED: Duplicate /voice-to-video route - now handled by video_routes.py
# This was causing route conflicts and inconsistent avatar loading
# The correct implementation with proper avatar query is in video_routes.py
