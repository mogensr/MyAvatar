...
async def create_video_from_text_input(request: Request):
    """Create video from text using HeyGen text-to-speech - WITH DEBUGGING"""
    
    # Debug: Log all form data first
    try:
        form_data = await request.form()
        log_info(f"[Video] Received form data keys: {list(form_data.keys())}", "Video")
        log_info(f"[Video] Form data values: {dict(form_data)}", "Video")
    except Exception as e:
        log_error(f"[Video] Error reading form data: {e}", "Video")
        return JSONResponse(status_code=400, content={"error": "Invalid form data"})
    
    # Now try to extract the individual fields
    try:
        title = form_data.get("title")
        avatar_id = form_data.get("avatar_id") 
        text = form_data.get("text")
        description = form_data.get("description")
        format_val = form_data.get("format", "16:9")
        
        log_info(f"[Video] Extracted fields - Title: '{title}', Avatar: '{avatar_id}', Text length: {len(text) if text else 0}, Format: '{format_val}'", "Video")
        
        # Check for missing required fields
        if not title:
            return JSONResponse(status_code=422, content={"error": "Title is required"})
        if not avatar_id:
            return JSONResponse(status_code=422, content={"error": "Avatar ID is required"})
        if not text:
            return JSONResponse(status_code=422, content={"error": "Text is required"})
            
    except Exception as e:
        log_error(f"[Video] Error extracting form fields: {e}", "Video")
        return JSONResponse(status_code=422, content={"error": "Invalid form fields"})
