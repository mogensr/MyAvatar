...
try:
    ...
    log_error("PostgreSQL support is NOT available. This application requires PostgreSQL.", "Database")
    raise ImportError("PostgreSQL support is required. Install psycopg2.")
...
if not USE_POSTGRES:
    log_error("PostgreSQL DATABASE_URL not set or psycopg2 not available. This application requires PostgreSQL.", "Database")
    raise RuntimeError("PostgreSQL DATABASE_URL not set or psycopg2 not available.")
...
def get_db_connection():
    """Get a PostgreSQL database connection (Postgres only)"""
    try:
...
def init_database():
    ...
    try:
        ...
        CREATE TABLE IF NOT EXISTS videos (
            ...
            input_url TEXT,  -- Added for BackgroundFX compatibility
...
def update_database_schema():
    ...
    try:
        ...
        # VIDEOS: Add input_url column if it doesn't exist
        # Check for input_url in videos
        check_input_url = """
        SELECT column_name FROM information_schema.columns WHERE table_name='videos' AND column_name='input_url'"""
        input_url_result = execute_query(check_input_url, fetch_all=True)
        if not input_url_result:
            log_info("Adding input_url column to videos table", "Database")
            execute_query("ALTER TABLE videos ADD COLUMN input_url TEXT")
            log_info("input_url column added to videos table", "Database")
        else:
            log_info("input_url column already exists in videos table", "Database")
        # Check if column exists (user_avatars)
        check_column = """
        SELECT column_name 
        ...
        # Check for voice parameters columns in user_avatars
        voice_params_columns = [
        ...
        for column in voice_params_columns:
            check_column_sql = f"""
            SELECT column_name 
            FROM information_schema.columns 
            WHERE table_name='user_avatars' AND column_name='{column}'
            """
            column_result = execute_query(check_column_sql, fetch_all=True)
            
            if not column_result:
                log_info(f"Adding {column} column to user_avatars table", "Database")
                
                if column == "voice_parameters_analyzed":
                    execute_query(f"ALTER TABLE user_avatars ADD COLUMN {column} BOOLEAN DEFAULT FALSE")
                elif column in ["voice_parameters_speed", "voice_parameters_pitch"]:
                    execute_query(f"ALTER TABLE user_avatars ADD COLUMN {column} FLOAT DEFAULT 1.0")
                else:  # voice_parameters_emotion
                    execute_query(f"ALTER TABLE user_avatars ADD COLUMN {column} TEXT DEFAULT 'Friendly'")
                    
                log_info(f"{column} column added to user_avatars table", "Database")
            else:
                log_info(f"{column} column already exists in user_avatars table", "Database")
