...
def initialize_backgrounds_schema(db_path):
    """
    Initialize the backgrounds table schema
    ...
    try:
        ...
        cursor.execute(CREATE_BACKGROUNDS_TABLE)
        
        # Check if background_id column exists in videos table
        cursor.execute("PRAGMA table_info(videos)")
        columns = [column[1] for column in cursor.fetchall()]
        
        # Only alter videos table if background_id column doesn't exist
        if 'background_id' not in columns:
            cursor.execute(ALTER_VIDEOS_TABLE)
            logger.info("Added background_id column to videos table")
        
        conn.commit()
        conn.close()
        logger.info("Backgrounds schema initialized successfully")
        return True
        
    except sqlite3.Error as e:
        logger.error(f"Database error initializing backgrounds schema: {e}")
        return False
    except Exception as e:
        logger.error(f"Error initializing backgrounds schema: {e}")
        return False
