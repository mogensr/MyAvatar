...
Updated: 2025-08-13 - GPU-OPTIMIZED: Proper CUDA utilization and memory management
"""
...
try:
    # Force GPU-accelerated rembg
    from rembg import remove, new_session
    REMBG_AVAILABLE = True
    logger.info("✅ Rembg loaded")
    
    # Initialize GPU-accelerated session
    if CUDA_AVAILABLE:
...
# Background loading functions (same as before)
...
def get_basic_backgrounds():
    """Get basic background collection"""
    return {
...
# GPU-OPTIMIZED: Enhanced segmentation functions
...
def segment_person_rembg_optimized(frame):
    """Optimized rembg segmentation with better memory management"""
    try:
