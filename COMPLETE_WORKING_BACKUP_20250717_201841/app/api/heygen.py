...
NEW: Enhanced avatar detection with old talking photo support
"""
import requests
import json
...
#####################################################################
# CHAPTER 1: ENHANCED AVATAR DETECTION AND CONFIGURATION
#####################################################################

def get_avatar_from_any_endpoint(api_key: str, avatar_id: str):
    """
    Fetch avatar using HeyGen API endpoints
    Checks both regular avatars and photo avatars since they use different endpoints
    
    Args:
        api_key: HeyGen API key
        avatar_id: Avatar ID to fetch
        
    Returns:
        Dictionary with avatar details and type, or None if not found
    """
    headers = {
        "X-Api-Key": api_key,
    ...
    try:
        # First, try the photo avatar endpoint directly
        ...
        # If not found as photo avatar, check regular avatars endpoint
        log_info(f"Checking regular avatars endpoint for {avatar_id}", "HeyGen API")
        ...
        if response.status_code != 200:
            log_error(f"HeyGen API returned status {response.status_code}: {response.text}", "HeyGen API")
            return None
        
        data = response.json()
        
        if data.get("error"):
            log_error(f"HeyGen API error: {data.get('error')}", "HeyGen API")
            return None
        
        # Search through all avatars for the requested ID
        ...
        for avatar in avatars:
            if avatar.get("avatar_id") == avatar_id:
                ...
                # Determine avatar type based on available fields
                avatar_type = "stock"  # Default
                
                # Check if it's a custom/instant avatar (usually has different naming patterns)
                if len(avatar_id) == 32 or "-" not in avatar_id:
                    avatar_type = "instant"
        ...
        # Check talking photos section if it exists
        ...
        for photo in talking_photos:
            ...
            if photo_id == avatar_id:
                log_info(f"Found talking photo {avatar_id}", "HeyGen API")
                return {
                    "type": "talking_photo",
                    "data": photo,
                    "config": {
        ...
        # Avatar not found in any endpoint
        log_warning(f"Avatar {avatar_id} not found in any HeyGen endpoint", "HeyGen API")
        return None
        
    except requests.exceptions.Timeout:
        log_error(f"Timeout while fetching avatar {avatar_id} from HeyGen API", "HeyGen API")
        return None
    except requests.exceptions.RequestException as e:
        log_error(f"Request error while fetching avatar {avatar_id}: {e}", "HeyGen API")
        return None
    except Exception as e:
        log_error(f"Unexpected error while fetching avatar {avatar_id}: {e}", "HeyGen API")
        return None

def get_character_config(api_key: str, avatar_id: str):
    """
    ENHANCED: Auto-detect avatar type and return appropriate character config
    ...
    try:
        ...
        # Use enhanced avatar detection
        avatar_info = get_avatar_from_any_endpoint(api_key, avatar_id)
        
        if avatar_info:
            if avatar_info.get("config"):
                log_info(f"Detected {avatar_info['type']} avatar: {avatar_id}", "HeyGen API")
                return avatar_info["config"]
            elif avatar_info.get("type") == "old_talking_photo":
                log_error(f"Old talking photo detected: {avatar_id}", "HeyGen API")
                raise ValueError(f"Avatar {avatar_id} is from the old talking photo system and needs to be re-uploaded")
        
        # Fallback: assume regular avatar
        log_warning(f"Avatar detection failed for {avatar_id}, assuming regular avatar", "HeyGen API")
        return {
            "type": "avatar",
            "avatar_id": avatar_id,
            "avatar_style": "normal"
...
def create_video_from_audio_file(api_key: str, avatar_id: str, audio_url: str, video_format: str = "16:9"):
    """
    Create a video using an audio file URL - ENHANCED VERSION with Photo Avatar Support
    ...
    # ENHANCED: Auto-detect if this is a photo avatar
...
#####################################################################
# CHAPTER 3: VIDEO CREATION WITH TEXT-TO-SPEECH
#####################################################################

def create_video_from_text(api_key: str, avatar_id: str, text: str, video_format: str = "16:9", voice_id: str = None):
    """
    Create video using text-to-speech with HeyGen API v2 - ENHANCED WITH PHOTO AVATAR SUPPORT
...
#####################################################################
# CHAPTER 5: PHOTO AVATAR MANAGEMENT
#####################################################################
...
def create_video_with_background(api_key: str, avatar_id: str, audio_url: str, background: dict, video_format: str = "16:9"):
    """
    Create video with custom background - ENHANCED VERSION with Photo Avatar Support
...
#####################################################################
# CHAPTER 7: INFORMATION AND STATUS FUNCTIONS
#####################################################################
