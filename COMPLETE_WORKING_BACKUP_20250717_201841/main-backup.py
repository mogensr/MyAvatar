...
========================================================
Modular version with enhanced organization - REFACTORED ROUTES + PREMIUM FEATURES
"""
...
# =============================================================================
# MODULAR ROUTE IMPORTS - REFACTORED STRUCTURE + PREMIUM FEATURES
# =============================================================================
...
modular_route_imports = [
  ...
  # PREMIUM ROUTES - NEW PREMIUM BACKGROUND REPLACEMENT SYSTEM
  ("backend.routers.premium", "router", None),  # No prefix since it already has /api/premium
...
async def debug_routes():
  ...
  return {
      ...
      "premium_features": {
          "premium_router_loaded": "backend.routers.premium" in str(routers_loaded),
          "premium_endpoints_available": len([r for r in routes if "/api/premium" in r.get("path", "")]),
          "database_tables_created": True  # Premium tables were created via migration
      },
...
async def test_route():
  ...
  return {
      "message": "FastAPI is working with Premium Features!", 
      ...
      "modular_structure": {
          ...
          "premium_routes": "/api/premium/* (NEW - Background Replacement)",
          "video_routes": "/voice-recording, /text-to-video (ENABLED)", 
          "emergency_routes": "/emergency/*"
      },
      "premium_endpoints": {
          "trial_start": "/api/premium/trial/start",
          "subscription_check": "/api/premium/subscription/check-access",
          "background_upload": "/api/premium/backgrounds/upload",
          "background_replacement": "/api/premium/background-replacement",
          "stock_images": "/api/premium/stock-images/search"
      },
      "video_url_refresher": {
...
# =============================================================================
# PREMIUM SYSTEM STATUS ENDPOINT
# =============================================================================

@app.get("/admin/premium-status")
async def premium_system_status():
  """Check premium system status for admin"""
  try:
      from backend.db import get_db
      from backend.models.premium import UserSubscription, PremiumFeature
      from sqlalchemy.orm import Session
      
      # Get database session
      db = next(get_db())
      
      # Check premium tables
      subscription_count = db.query(UserSubscription).count()
      feature_count = db.query(PremiumFeature).count()
      ...
      return {
          ...
          "database_tables": {
              ...
              "premium_features": feature_count,
              "tables_created": True
          },
          "integrations": {
  ...
  try:
      ...
      download_data = {
          "metadata": {
              ...
              "myavatar_version": "premium_edition"
          },
...
async def debug_environment():
  ...
  try:
      ...
      try:
          from backend.models.premium import UserSubscription
...
async def startup_event():
  ...
  try:
      from backend.models.premium import UserSubscription, PremiumFeature
      logger.info("✅ Premium system models loaded successfully")
  except Exception as e:
  ...
  log_info(f"MyAvatar {edition_name} is running with MODULAR ROUTE STRUCTURE + PREMIUM FEATURES", "Server")
  ...
  # Check if premium router loaded
  premium_loaded = any("premium" in r for r in routers_loaded)
  if premium_loaded:
      logger.info("🎯 PREMIUM FEATURES ACTIVE: Background replacement system ready")
  else:
      logger.warning("⚠️ Premium features not loaded - check router configuration")
...
# IMPORT SMS MONITORING
try:
    from sms_monitoring import SMSMonitor
    sms_monitor_available = True
    logger.info("SMS monitoring imported successfully")
except ImportError as e:
    logger.warning(f"SMS monitoring not available: {e}")
    sms_monitor_available = False

# START SMS MONITORING
if sms_monitor_available:
    try:
        sms_monitor = SMSMonitor()
        # Update phone number - REPLACE WITH YOUR ACTUAL NUMBER
        sms_monitor.phone_number = "+45XXXXXXXXX"  # Replace with your Danish number
        
        # Start monitoring
        monitor_thread = sms_monitor.start_monitoring()
        logger.info("✅ SMS monitoring started")
    except Exception as e:
        logger.error(f"Failed to start SMS monitoring: {e}")
        sms_monitor_available = False
else:
    logger.warning("⚠️ SMS monitoring not started")

# SMS TEST ENDPOINTS
@app.get("/admin/test-sms")
async def test_sms_monitoring():
    """Test SMS monitoring functionality"""
    try:
        if not sms_monitor_available:
            return {"error": "SMS monitoring not available"}
        
        result = sms_monitor.test_sms()
        return {
            "success": result,
            "message": "SMS test completed" if result else "SMS test failed",
            "phone_number": sms_monitor.phone_number,
            "service": sms_monitor.sms_service
        }
    except Exception as e:
        return {"error": str(e)}

@app.get("/admin/sms-status")
async def sms_monitoring_status():
    """Check SMS monitoring status"""
    if not sms_monitor_available:
        return {"available": False, "error": "SMS monitoring not available"}
    
    return {
        "available": True,
        "phone_number": sms_monitor.phone_number,
        "service": sms_monitor.sms_service,
        "check_interval": sms_monitor.check_interval,
        "alert_cooldown": sms_monitor.alert_cooldown,
        "app_currently_down": sms_monitor.is_app_down,
        "last_alert_time": sms_monitor.last_alert_time
    }
