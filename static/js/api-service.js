...
// Central API-service til MyAvatar-backend - Version 2.2
// Enhanced with improved HeyGen integration and error handling

console.log('=== API Service V2.2 Loading ===');
...
class APIService {
  ...
  // Enhanced voice parameters fetching with proper error handling
  static async getVoiceParameters(avatarId) {
    console.log(`Getting voice parameters for avatar ${avatarId}...`);
    try {
      const response = await this._json(`/api/avatar/${avatarId}/voice-parameters`);
      
      if (!response.success) {
        throw new Error(response.error || 'Failed to fetch voice parameters');
      }
      
      return response.parameters;
    } catch (error) {
      console.error(`Error fetching voice parameters for avatar ${avatarId}:`, error);
      
      // Return default parameters as fallback
      console.warn('Using default voice parameters as fallback');
      return {
        emotion: 0.5,
        speed: 1.0,
        pitch: 1.0,
  ...
  static async createVideo(videoData) {
    ...
    try {
      const response = await this._json('/api/videos/create', {
        method: 'POST',
  ...
  static async getVideoStatus(videoId) {
    ...
    try {
      const response = await this._json(`/api/videos/${videoId}/status`);
      
      if (!response.success) {
        throw new Error(response.error || `Failed to get status for video ${videoId}`);
      }
  ...
  // Get all user videos with refresh option
  static async getUserVideos(forceRefresh = false) {
    console.log('Getting user videos' + (forceRefresh ? ' (forced refresh)' : ''));
    try {
      const endpoint = forceRefresh ? '/api/videos?refresh=true' : '/api/videos';
      const response = await this._json(endpoint);
